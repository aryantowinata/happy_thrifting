...
class CartController extends Controller
{
    ...
    public function buyNow(Request $request, $id)
    {
        ...
        if ($product->jumlah_produk < 1) {
            return redirect()->route('product_page')->with('error', 'Product is out of stock.');
        }
        ...
        $carts = Cart::with('products')->where('id_user', $user->id)->get();
    ...
    public function checkout(Request $request)
    {
        ...
        foreach ($carts as $keranjang) {
            $total_harga += $keranjang->products->harga_produk * $keranjang->jumlah;
        }
        ...
        foreach ($carts as $keranjang) {
            ...
            OrderItem::create([
                ...
                'harga_satuan' => $keranjang->products->harga_produk,
            ]);

            $this->updateProductQuantity($keranjang->id_product, $keranjang->jumlah);
        }
        ...
        foreach ($carts as $keranjang) {
            $itemDetails[] = [
                ...
                'price' => $keranjang->products->harga_produk,
                'quantity' => $keranjang->jumlah,
                'name' => $keranjang->products->nama_produk,
            ];
        }
        ...
        $carts = Cart::with('products')->where('id_user', $userId)->get();

        // Hitung total harga keranjang
        $total_harga = $carts->sum(function ($cart) {
            return $cart->products->harga_produk * $cart->jumlah;
        });
     ...
     *
     * @param int $id_product ID produk yang jumlahnya ingin diperbarui.
